...
# BEÁLLÍTÁSOK
# ==========================================================
input_file <- Sys.getenv("SQANTI_LENGTH_STATS_FILE", unset = "sqanti_length_boxplot_stats.tsv")
...
header_text <- "Transcript length distributions across annotators"
header_size_pt <- 14
header_bold <- TRUE
header_fill <- "grey90"
...
cell_levels  <- c("H1","H1-endo","WTC11")
chem_levels  <- c("ONT-dRNA","ONT-cDNA","ONT-CapTrap","PacBio","PacBio-CapTrap")
annot_levels <- c("LoRTIA-Plus","bambu","FLAIR","IsoQuant","NAGATA","StringTie2")
...
canon_annotator <- function(x) {
  xlow <- str_to_lower(x)
  dplyr::case_when(
    xlow %in% c("lortia", "lortia-plus") ~ "LoRTIA-Plus",
    xlow == "flair"    ~ "FLAIR",
    xlow == "isoquant" ~ "IsoQuant",
    xlow == "nagata"   ~ "NAGATA",
    xlow == "bambu"    ~ "bambu",
    xlow %in% c("stringtie", "stringtie2") ~ "StringTie2",
    TRUE ~ x
  )
}
...
parse_meta_from_file <- function(file_vec) {
  ...
  cell      <- str_extract(f, "^(H1-endo|H1|WTC11)")
  chemistry <- str_extract(f, "(ONT-dRNA|ONT-cDNA|ONT-CapTrap|PacBio-CapTrap|PacBio)")
  annotator <- str_extract(f, regex("(LoRTIA-Plus|LoRTIA|FLAIR|IsoQuant|NAGATA|bambu|StringTie2)", ignore_case = TRUE))
...
if (all(c("Annotator", "Chemical", "Cell") %in% names(df0))) {
  meta <- df0 %>%
    transmute(
      cell = as.character(Cell),
      chemistry = str_replace_all(as.character(Chemical), regex("captrap", ignore_case = TRUE), "CapTrap"),
      annotator = canon_annotator(as.character(Annotator)),
      file_norm = paste(cell, chemistry, annotator, "classification", sep = "-")
    )
} else {
